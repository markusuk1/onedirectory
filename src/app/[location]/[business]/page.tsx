...
export async function generateMetadata({
  ...
  params: Promise<{ location: string; business: string }>;
}): Promise<Metadata> {
  const { location, business: businessSlug } = await params;
  const biz = getBusinessBySlug(location, businessSlug);
  if (!biz) return {};
  const ratingStr = biz.rating ? ` | ${biz.rating}/5 (${biz.totalReviews} reviews)` : "";
  return {
    title: `${biz.name} - ${biz.locationName}${ratingStr}`,
    description: `${biz.name} provides minibus and coach hire in ${biz.locationName}. ${biz.address}. Contact details, reviews and free quotes.`,
  };
}
...
export default async function BusinessPage({
  ...
  const { location: locationSlug, business: businessSlug } = await params;
  const business = getBusinessBySlug(locationSlug, businessSlug);
  if (!business) notFound();

  const location = getLocationBySlugWithCount(locationSlug);
  const nearbyBusinesses = getBusinessesByLocation(locationSlug)
    ...
    <>
      ...
      <nav className="bg-surface border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 text-sm text-text-light">
          ...
          <Link href={`/${locationSlug}`} className="hover:text-primary">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white border border-border rounded-xl p-6 mt-6 space-y-4">
              ...
              <div className="flex items-start gap-3">
                <svg className="w-5 h-5 text-text-light shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="sticky top-4 space-y-4">
              ...
              <div className="bg-surface border border-border rounded-xl p-5">
                <p className="text-sm text-text-light">
                  ...
                  , which has {location?.businessCount} minibus and coach hire
                  companies.
                </p>
              </div>
            </div>
